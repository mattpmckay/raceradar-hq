    ...
    <section className="relative pb-24 pt-4">
      <div className="container-page">
        ...
        <div className="mb-10 flex items-end justify-between">
          <div>
            <div className="mb-2 h-3 w-20 animate-pulse rounded bg-wire" />
            <div className="h-9 w-56 animate-pulse rounded-lg bg-wire" />
          </div>
          <div className="hidden h-5 w-28 animate-pulse rounded bg-wire sm:block" />
        </div>

        {/* Filter strip */}
        <div className="mb-10 flex gap-2 overflow-hidden">
          {Array.from({ length: 7 }).map((_, i) => (
            <div key={i} className="h-9 w-20 shrink-0 animate-pulse rounded-full bg-wire" />
          ))}
        </div>
