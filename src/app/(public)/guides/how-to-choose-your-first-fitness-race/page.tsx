...
export default function HowToChooseYourFirstRacePage() {
  return (
    <div className="container-page py-10">
      ...
      {/* Back */}
      <Link href="/guides" className="btn-ghost mb-6 inline-flex px-0 text-gray-400">
      ...
      <div className="mb-10 max-w-2xl">
        <div className="flex items-center gap-3 mb-4">
          <span className="text-xs font-semibold uppercase tracking-widest text-gray-500">Getting Started</span>
      ...
      <div className="max-w-2xl space-y-10">
        ...
        {/* Step 1 */}
        <section>
          <h2 className="text-lg font-bold text-white mb-4 pb-3 border-b border-surface-border">
          ...
          <ul className="space-y-3">
            {[
              ...
              <li key={q} className="rounded-xl border border-surface-border bg-surface-card p-4 text-sm space-y-1">
        ...
        <section>
          ...
          <div className="space-y-3 text-sm">
            {[
              ...
              <div key={window} className="flex gap-4 rounded-xl border border-surface-border bg-surface-card p-4">
                <div className="text-brand-400 font-semibold shrink-0 w-28">{window}</div>
            ...
            <table className="w-full text-sm border-collapse">
              <thead>
                <tr className="border-b border-surface-border">
                  {['Discipline', 'Race time', 'Training focus', 'Cost (entry)', 'Best for'].map((h) => (
                    <th key={h} className="text-left text-xs font-semibold text-gray-500 uppercase tracking-wider pb-3 pr-4 first:pl-4 sm:first:pl-0 whitespace-nowrap">
                      {h}
                    </th>
                  ))}
                </tr>
              ...
              <tbody className="divide-y divide-surface-border">
                {[
                  ...
                  <tr key={row.name} className="hover:bg-surface-card/50 transition-colors">
                    <td className="py-3 pr-4 pl-4 sm:pl-0 font-medium text-white">{row.name}</td>
                    <td className="py-3 pr-4 text-gray-400 whitespace-nowrap">{row.time}</td>
                    <td className="py-3 pr-4 text-gray-400">{row.training}</td>
                    <td className="py-3 pr-4 text-gray-400 whitespace-nowrap">{row.cost}</td>
                    <td className="py-3 text-gray-400">{row.for}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="grid gap-4 sm:grid-cols-3 text-sm">
            {[
              ...
              <div key={type} className="card space-y-2">
                ...
                <div className="text-brand-400 text-xs">{events}</div>
                <div className="text-gray-500 text-xs leading-relaxed">{note}</div>
              </div>
            ))}
          </div>
        </section>
        ...
        <section>
          ...
          <div className="space-y-3 text-sm">
            {[
              ...
              <div key={condition} className="rounded-xl border border-surface-border bg-surface-card p-4 flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-3">
                <div className="text-gray-400 sm:flex-1">{condition}</div>
                <div className="text-brand-400 font-medium sm:text-right">{action}</div>
              </div>
            ))}
          </div>
        ...
        {/* Final tip */}
        <section className="rounded-2xl border border-brand-500/20 bg-brand-500/5 p-6 text-sm">
          <h3 className="font-semibold text-white mb-2">One more thing: register early</h3>
        ...
        <div className="rounded-2xl border border-surface-border bg-surface-card p-8 text-center">
          <h2 className="text-lg font-bold text-white">Find your first race</h2>
